% Converting the .mcd file to matlab data set.
...
function convres=mc2mat(fileName,varargin)
%%%%%%%%%%%% Basic setting 
%%% parameter default
blockSize=0.2; % read block size (ratio to the available memory)
bOverWrite=false;
bPass=false;
bServer=false;

convres=false; % the success return flag initialtion
    ...
    for k=1:nargin-1
        switch varargin{k}
            ...
            case 'server'
                ...
                bServer=true;
%             case 'logf' % the msgfile of log file
%                 msgfile=varargin{k+1};
            otherwise
                % dismiss, because it may be the msgfile after 'logf'.
        end
    end
end
...
% memory to be used
blockSize=mem*blockSize; % (byte)
fprintf('memory to be used: %0.2g GB\n',blockSize/1024^3);
% fprintf(msgfile,'memory to be used: %0.2g GB\n',blockSize/1024^3);


%%% Set output mat file for storing
outputFileName=[fp,outputFileName,'.mat'];

%%% Set input file and proceed
if intype==1
    inputFileName=[fp,fn,'.mcd'];
    convres=mcd2mat(outputFileName,inputFileName,bServer); 
else
    inputFileName=[fp,fn,'.txt'];
    convres=mcdtxt2mat(outputFileName,inputFileName,bServer,blockSize);
end
...
%%%%%%%%%%%%%%
% convert from .mcd file directly
%%%%%%%%%%%%%%
function convres=mcd2mat(outputFileName,inputFileName,bServer)
%%% Get input file information and open file
[info,etData]=getMCDinfo(inputFileName,bServer); % * getMCDinfo also set neuroshare library
...
outfile.srate=info.srate;
outfile.T=(1:info.ptsAmt)'/info.srate; % measured in (s)
outfile.X=zeros(info.ptsAmt,info.rawchAmt,'double');

%%% Read data channel from source file.
fprintf('total number of channels: %d\n',info.rawchAmt);
for k=1:info.rawchAmt
    % use "GetAnalogData" option
    [~,~,enData]=ns_GetAnalogData(infile,info.dataChEntity(k),1,info.ptsAmt);
%     if count~=info.ptsAmt, error('read length error'); end
    outfile.X(:,k)=enData; % unit:(V)
    fprintf('|'); % visualization of progress
end
fprintf('\n');
info.Yunit='mV';
...
if info.bStim
    outfile.E=etData(:,info.IstimIdx);
end
...
%%%%%%%%%%%%%%
% convert from .mcd-derived txt file
%%%%%%%%%%%%%%
function convres=mcdtxt2mat(outputFileName,inputFileName,bServer,blockSize)
...
% fprintf(msgfile,'Total data size: %d lines (%2.2g GB)\n',info.lineAmt,tp);

% turn the block size unit to (line)
blockSize=floor(blockSize/info.lineLen);
fprintf('The processing block size: %d lines \n',blockSize);
% fprintf(msgfile,'The processing block size: %d lines \n',blockSize);
...
% fprintf(msgfile,'sampling rate: %d\n',srate);


%%% write important information to output
outfile.srate=srate;
temp=struct('chAmt',info.rawchAmt,'chID',info.chID,'ptsAmt',info.lineAmt);
outfile.info=temp;
...
ptsAmt=info.lineAmt;
[bSeg,bAmt]=cutseg([1,ptsAmt],blockSize);
fprintf('total number of blocks: %d\n',bAmt);
% fprintf(msgfile,'total number of blocks: %d\n',bAmt);

% move to location of read start
fseek(infile,info.dStLoc,-1);
% initialize the output
outfile.X=zeros(1,info.rawchAmt,'double');
