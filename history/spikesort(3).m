...
sna=length(runOpt);
% 输入只有一项的情况下（‘resume'必须只能有一项）
if sna==1 
    if strcmp(runOpt{1},'resume') % differentiate the "resume" or not situations
        bResume=true;
    else        
...
% * If resume, restore the run status saved last time.
if bResume
    ...
    if exist('runstat','var')
        % steps upto the one before runstat recorded are passed
        runFlag(1 : rs2pgnum(runstat,funcNumTab)-1)=false;
    else
        error('can not find the run status in resume mode');
    end
    
    % Make direct memory option tuned to current machine, even in 'resume' mode.
    paras.bDirectMem=bDirectMem;
    save(sspath,'-append','paras');

%%% Necessary initiation if run from new
else    
    % Information of data    
...
%     save(sspath,'-append','runstat');    
    ...
    for chi=1:info.rawchAmt
        [SD{chi},SQ{chi},SA{chi}]=spike_detect(outfile1.X(:,chi),info.srate);        
        fprintf('|');
    end
    fprintf('\n');    
...
%     if bDirectMem
%         [A,rmlist]=spike_align(outfile1.X,outfile2.SD,info.srate,'chAssign',outfile2.info.chID,'window',paras.alignWin);
%     else % 由于matfile方法处理2D矩阵的限制？
%         [A,rmlist]=spike_align(outfile1,outfile2.SD,info.srate,'chAssign',outfile2.info.chID,'window',paras.alignWin);
%     end
%     if ~isempty(rmlist)
%         for k=1:size(rmlist,1)
...
%     filerun('cluster',fnC,fnFEA,fnS,T);
    ...
    % Channel SI: record cluster identity of each spike in original channel.
    CSI=cell(info.chAmt,1); % * note there may be 0 in CSI - those not assigned to any cluster.       
    % record from which raw channel (electrode) the neuron is recorded.
    newchID=zeros(0,1);
    ...
    for chi=1:info.chAmt
        tp=length(SD{chi});
        if tp<=cluMinSpkThres
            chcount=chcount+1;
            newSD{chcount,1}=SD{chi};            
            newchID(chcount,1)=info.chID(chi);
            CSI{chi}=zeros(tp,1);
        else
            [CSI{chi},lbinfo]=spike_cluster(SF{chi,1},'kmeans',cluMinSpkThres);%,SD{chi},outfile2.A{chi,1});
            if lbinfo.cAmt>0 % * it is possible that non is left in clustering. 
                for k=1:lbinfo.cAmt
                    chcount=chcount+1;
    ...
    %%% Apply the separation of spike trains to new data.    
    % CSI to raw channels
    ...
    % channel ID & chAmt
    info.chID=newchID;
    info.chLabel=info.rawchLabel(info.chID);
    info.chAmt=chcount;
    SD=newSD;
    
    % Save output
    save(fnC,'-v7.3','-append','time','rawSD','SD','CSI','info');
