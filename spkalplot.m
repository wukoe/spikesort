...
function varargout=spkalplot(X,varargin)
%%% Parameters default
...
pgAmt=ceil(chAmt/pageplotnum);
pgCh=cutseg([1,chAmt],pageplotnum);
...
%         tp=pgCh(pgi,2)-pgCh(pgi,1)+1; % number of subplot in this page.
        [rn,cn]=subpnum(pageplotnum); % get column and row number for best subplot layout
